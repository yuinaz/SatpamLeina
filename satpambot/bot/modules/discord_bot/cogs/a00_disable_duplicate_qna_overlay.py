...
ENABLE = os.getenv("DISABLE_DUPLICATE_QNA", "1") != "0"  # default ON
    ...
    async def on_message(self, message):
        ...
        if prev and prev[0] == h and (now - prev[1]) <= WINDOW_SEC:
            try:
                ...
                log.warning("[dup-qna] dropped duplicate in %s", key)
            except Exception as e:
                log.warning("[dup-qna] delete failed: %s", e)
            return
...
async def setup(bot: commands.Bot):
    ...
    try:
        ...
        logging.getLogger(__name__).error("[dup-qna] setup failed: %s", e)